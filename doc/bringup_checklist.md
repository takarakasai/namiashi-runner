...
**`--forever` を付けると Ctrl-C まで回り続ける**（`--secs` より優先）。
...
`--forever` なら秒数ではなく **Ctrl-C で確定**する。打ち切っても集計と `--write` は
そのまま走るので、「納得したら止める」使い方ができる。
...
> エンコーダのゼロがずれる**。したがって `calib zero` は**必ず段階 5 の後**に行う。
...
1. 画面に各関節の目標角が表示される
2. **ロボットを手でその姿勢に保持する**
3. Enter → 全軸ゼロ出し → `zero_pose_rad` が記録される

**合格条件:**
- [ ] 保持した姿勢が表示された角度と目視で一致している
- [ ] 「ゼロ出し完了」が出る（3 秒以内にアンカーできない場合はモータ電源とボーレート）
- [ ] `zero_pose_rad` が書き戻された
