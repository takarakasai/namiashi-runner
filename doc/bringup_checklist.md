...
> **`--allow-no-sbus` は受信が無い＝フェイルセーフとして扱われ、その中身は
> `ModeRequest::Stand`。** つまり**起動した瞬間に、止める操作なしで立ち上がる**。
> CH5 で脱力に留める手段はない。脚を浮かせてからでなければ回さないこと。

**遷移の起点は伏せ姿勢ではなく「いまの実測値」。** 可動域測定でメカ端まで
振った直後などは、そこから `constrain` まで一気に動く。`run` の前に
...
- [x] **CH5 が「脱力」位置**で起動する
- [x] 脚を浮かせたまま CH5 を「起立」へ → 姿勢を確認
- [ ] スティック操作が期待どおりの軸に効く ← **vy 反転を修正、再確認待ち**
...
| CH2 前後 (vx) | ✅ |
| CH4 旋回 (wz) | ✅ |
| **CH1 左右 (vy)** | ❌ **逆だった** → `[teleop.vy] reverse` を `true`→`false`。**再確認待ち** |
...
| 8 プロポ操縦 | 2026-08-22 | ⚠ | 脱力 / 起立 / 歩容開始・フェイルセーフ・Ctrl-C 脱力を確認。**vy が逆**だったので設定を反転（再確認待ち）。**CH3 高さは歩容側が未対応**で効かない |
