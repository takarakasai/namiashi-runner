...
> **2026-08-21 追記: このパッチはビルドに入っていない。**
> `dev-siblings.sh` はオフで、依存は GitHub の `5db57cf`（未パッチ）を取っている。
> ローカルの `misa-actuator` 作業ツリーにしか無いので、**`cargo build` した
> バイナリは未パッチのまま**である。取り込むならコミットして push すること。
>
> なお後述の「無応答時に 1 周 10 秒」はこのパッチとは**無関係**だった。
> `response_timeout_ms` を 20 → 200 に変えても総時間が 1.3 秒しか動かず、
> 受信側は設定どおり効いていることを確認済み。
